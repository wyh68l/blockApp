<template>
    <div class="page">
        <m-header :title="language=='zh-cn'? '我的买单':allTxt&&allTxt['myBill']" :canback="true"></m-header>
        <section class="body">
            ...
            <footer class="f-btns">
                <button class="btn gradient-origin" @click="addOrder">{{language=='zh-cn'?'创建买单':allTxt&&allTxt['creteBuy']}}</button>
            </footer>
        </section>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        addOrder(){
            this.$router.push({name:'AddOrder',params:{type:'buy'}})
        },
        //0全部  1待交易、2待付款、3待确认、4已完成、10申诉中  12订单失效
        getOrder(){
            let state = this.state==0?'':this.state;
            Trade.getOrder({page:this.page,type:1,state,devide:1}).then(res=>{
                if(res.code == 1){
